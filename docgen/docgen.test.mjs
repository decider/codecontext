...
test('selectFiles skips files carrying an @generated marker', () => {
  ...
  // regenerated -> its mirror is rewritten -> the hash changes -> regenerate.
  // Observed live: 9/9 mirrored dirs in pbx-platform never converged.
  const root = freshRepo();
  try {
    ...
    file(
      root,
      'pkg/CLAUDE.md',
      '<!-- @generated by pbx-autodocs-mirror — do not hand-edit -->\n## Purpose\nmirror\n',
    );
    file(
      root,
      'pkg/AGENTS.md',
    ...
    assert.ok(!files.includes('CLAUDE.md'), 'generated mirror must not be an input');
    assert.ok(!files.includes('AGENTS.md'), 'generated mirror must not be an input');
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});

test('selectFiles KEEPS hand-written CLAUDE.md / AGENTS.md', () => {
  // Excluding these by filename would drop real signal — a repo's root
  // CLAUDE.md is often the densest description of the system it has.
  // Ownership is decided by the marker, not the name.
  const root = freshRepo();
  try {
    file(root, 'pkg/CLAUDE.md', '# Ops rules\nNever write to mainnet without approval.\n');
    file(root, 'pkg/AGENTS.md', '# AGENTS.md\nGuidance for Codex.\n');

    const files = selectFiles(join(root, 'pkg')).map((f) => f.name);
    assert.ok(files.includes('CLAUDE.md'), 'curated CLAUDE.md is legitimate input');
    assert.ok(files.includes('AGENTS.md'), 'curated AGENTS.md is legitimate input');
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});

test('selectFiles only honours an @generated marker near the top of the file', () => {
  // Otherwise any file merely discussing the convention would exclude itself.
  const root = freshRepo();
  try {
    file(root, 'pkg/notes.md', '# Notes\n' + 'x'.repeat(4000) + '\n@generated mentioned in prose\n');
    const files = selectFiles(join(root, 'pkg')).map((f) => f.name);
    assert.ok(files.includes('notes.md'), 'a late mention must not exclude the file');
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
...
test('needsAnalysis: null when file content hashes match state', () => {
  ...
  try {
    ...
    // docgen writes the doc and the state entry TOGETHER; a state entry
    // with no doc on disk is the bug this suite now pins elsewhere.
...
test('a dir with a state entry but NO doc on disk is stale', () => {
  // The bug this pins: needsAnalysis used existsSync(docPath) only as a
  // fast-path SKIP. With the doc absent but a state entry present and files
  // unchanged, it fell through every check and returned null — "fresh".
  //
  // Measured on pbx-platform: 173 of 335 state entries had no AUTO_DOCS.md,
  // and all of them reported fresh. computeStatus said `uncovered=0` while
  // 52% of the tree was undocumented, and no runner could ever reach them
  // because --until-done only visits dirs needsAnalysis flags.
  const root = freshRepo();
  try {
    file(root, 'pkg/index.ts', 'export const x = 1;');
    const dir = join(root, 'pkg');
    // A state entry recording the CURRENT file hashes — i.e. nothing changed.
    const state = loadState(root);
    state.directories.pkg = {
      lastAnalyzed: new Date().toISOString(),
      reason: 'forced',
      version: '0.1.0',
      files: Object.fromEntries(selectFiles(dir).map((f) => [f.name, f.hash])),
      childVersionsAtAnalysis: {},
      curatedContextAtAnalysis: {},
    };
    saveState(root, state);

    assert.ok(!existsSync(join(dir, 'AUTO_DOCS.md')), 'precondition: no doc');
    const reason = needsAnalysis(root, dir, loadState(root));
    assert.ok(reason, 'a documented-in-state dir with no doc file must be stale');
    assert.match(reason, /doc-missing/);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});

test('a dir with a matching doc and unchanged files is still fresh', () => {
  // The fix must not make everything permanently stale.
  const root = freshRepo();
  try {
    file(root, 'pkg/index.ts', 'export const x = 1;');
    const dir = join(root, 'pkg');
    ...
    assert.equal(needsAnalysis(root, dir, loadState(root)), null);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
