...
quality_input_hash: de5c00bff9c8eef5
---
...
- `telemetry/cli.mjs` — `main()` + `parseArgs()`: the dashboard, dispatching to the render modes that consume the JSONL.
...
The writer is a gate, not a policy: `isDisabled()` short-circuits when `CLAUDE_TELEMETRY_DISABLED === '1'`, and every other failure is swallowed by the surrounding `try/catch` so telemetry can never block a hook's real context flow. The CLI's only decision is mode routing in `parseArgs()` — `--all`, `--session <sid>`, `--top`, `--hooks`, `--resets`, `--tail [N]`, defaulting to the newest session file by mtime; ranking modes (`renderTop`, `renderHooks`) sort descending by total `bytes_emitted`, breaking ties on event count.
...
| `event:'reset'` | `writer.mjs:writeEvent` | hook, sid, reason, dedup_files_removed | `cli.mjs:renderResets` | `sessions/<sid>.jsonl` |
...
- Session ids are filename-safe: `sanitiseSid()` strips to `[A-Za-z0-9._-]` and caps at 64 chars (`writer.mjs`).
