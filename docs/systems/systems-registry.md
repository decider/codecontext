...
`runOrganize`, `writeCompositeReadme`, and `buildSite` publish the manifests
plus a single-file HTML viewer.
...
flowchart TB
  ...
  subgraph Publish
    PR["prune orphans"]
    ORG["organize + composite README"]
    BLD["build static HTML"]
  end
  P0 -->|"candidates"| P1
  ...
  ORG -->|"_categories.json + README"| BLD
  BLD -->|"docs/systems.html"| HOST["Host repo (committed files)"]
  HOST -.->|"source edit triggers refresh"| P0
```
...
flowchart TB
  ...
  REG2 -->|"systems + mermaid"| SAN["sanitizeMermaid + validate-mermaid repair"]
...
- `systems-registry/run-pipeline.mjs` — orchestrates Pass 0→build in sequence; owns concurrency, token-budget metering, and orphan pruning
- `systems-registry/detect.mjs` — Pass 0; the 8-signal scorer + federation decomposition that produces candidates
- `systems-registry/pass2.mjs` — per-system body generator; anchor-file selection (`selectAnchorFiles`, `scoreFilename`) and the prompt the LLM sees
...
`buildSite`, `runOrganize`, and `writeCompositeReadme` write `docs/systems/*.md`,
`_categories.json`, `README.md`, and `docs/systems.html` back into the host
repo. The sibling `auto-docs-refresh` hook commits these onto the branch, and
...
|---|---|---|---|---|
| `status: active\|needs-review\|draft` | `pass2.mjs` / `pass26-revise.mjs` / `refine.mjs` (front-matter writes) | status | `registry.mjs:loadAll` (context-injection hook skips `draft`) | `docs/systems/<name>.md` |
...
| `docs/systems/README.md` | composite mermaid + table | `composite.mjs:writeCompositeReadme` | `build-static.mjs:loadComposite` | cross-system overview diagram |
| `docs/systems.html` | self-contained HTML | `build-static.mjs:buildSite` | browser (`file://`) | rendered viewer |
| run report | JSON | `run-pipeline.mjs` (write run report) | `view.mjs:readRunReport` | last-run scores + below-bar list shown on Overview |
...
- Mermaid syntax bugs are repaired best-effort by `validate-mermaid.mjs`; blocks still broken after `maxRounds` pass through unchanged.

## Where to start reading

1. `systems-registry/run-pipeline.mjs` — the orchestrator; the whole pass sequence and its cost controls live here
2. `systems-registry/cli.mjs` — command entry points (`detect`, `run`, `refresh`, `validate`, `refine`, `build`, `view`)
